...
export default function selectticket({token}) {
        ...
        setStatusBox(data?.createTicket.httpCode === '200' ? 
          {
            ...
            refCode: data?.createTicket.refCode
    ...
    const {data: data_seats, loading: loading_seats, error: error_seats, refetch: refetch_seats} = useQuery(GET_BOOKED_SEATS, {
      variables: {input: {_showID: router.query._showID}}
    })
    ...
    }, [data_show, data_seats]);

    // refetch data everytime routing to this page
    useEffect(() => {
      const handleRouteChange = () => {
        if (router.pathname === `/buyticket/${router.query._showID}`){
          refetch_show()
          refetch_seats()
        }
      }
    ...
    }, [router.pathname, refetch_show, refetch_seats])
...

        {
          ...
          <div style={{display: "flex"}}>
            <div style={{backgroundColor: "gray", width: "30%", lineHeight: "150%"}}>
              <Image width={"70%"} style={{margin:"10% 0 20%"}} src="error"
        ...
        <Modal centered title="" open={isStatusMordelOpen} okButtonProps={{style: {display: "none"}}} cancelButtonProps={{style: { display: 'none' }}} >
          {
            ...
            (
              <Result
                ...
                extra={
                  <div>
                    <p>User account: {statusBox.account} ฿</p>
                    <p>Reference Code: {statusBox.refCode}</p>
                    <br/>
                    <Button onClick={onClickStatusMordalContinue} >
                      CONTINUE BOOKING
                    </Button>
                    <Button onClick={onClickStatusMordalGoback}  type="primary">
                      GO BACK HOMEPAGE
                    </Button>
                  </div>
                }
